...
- **A result goes back the same way** (ADR 0010) — the dated `## Outcome` paragraph as `text`,
  ...
  one at landing. Ingest lists it as `sci-<n>` under `/desk`'s RESULTS (`R<n>` is a report). The
  ledger never writes a research file, and ingest never mints a task id.
- **Landing a result keeps the two authors apart** (2026-09-16). It is a Code session's job, in
  ...
  2. **Quote it verbatim** under the destination's `## Outcome` — the idea file's, or wherever
     ...
     `text`, headed `**<date> — sci-<n>, Claude Science session <frame id, 8 chars> (artifact <id>,
  ...
  4. Commit, then `ledger.py science triage sci-<n> --into <file>`.
...
name the `A<n>`/`sci-<n>` it became once the board shows it.
